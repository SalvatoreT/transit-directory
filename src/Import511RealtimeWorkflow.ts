...
> {
  async run(event: Readonly<WorkflowEvent<{}>>, step: WorkflowStep) {
    ...
    for (let i = 0; i < agencies.length; i++) {
      ...
      // --- Trip Updates ---
      const tripUpdatesResult = await step.do(
        `[${agency}] Fetch TripUpdates`,
        async () => {
          const url = `https://api.511.org/transit/tripupdates?api_key=${this.env.API_KEY_511}&agency=${agency}`;
          return await fetchAndDecodeFeed(url);
        },
      );

      // Update rate limit info from headers
      if (tripUpdatesResult.rateLimitLimit) {
        requestsAllowedInWindow = tripUpdatesResult.rateLimitLimit;
      }

      // Proactive sleep if we are out of quota
      if (tripUpdatesResult.rateLimitRemaining === 0) {
        await step.sleep(
      ...
      await step.do(`[${agency}] Process TripUpdates`, async () => {
        const { message } = tripUpdatesResult;
        ...
        if (!feedVersionId) {
          console.warn(`No active feed version for ${agency}. Skipping.`);
          return;
        }
      ...
      // --- Service Alerts ---
      const serviceAlertsResult = await step.do(
        `[${agency}] Fetch ServiceAlerts`,
        async () => {
      ...
      // Proactive sleep if we are out of quota
      if (serviceAlertsResult.rateLimitRemaining === 0) {
        await step.sleep(
      ...
      await step.do(`[${agency}] Process ServiceAlerts`, async () => {
        const { message } = serviceAlertsResult;
        const { feedSourceId, feedVersionId } = await getFeedContext(
          this.env.gtfs_data,
          agency,
        );
        ...
        if (alertsToClose.length > 0) {
          ...
          for (let j = 0; j < closeBatch.length; j += BATCH_SIZE) {
            await this.env.gtfs_data.batch(closeBatch.slice(j, j + BATCH_SIZE));
          }
        }
