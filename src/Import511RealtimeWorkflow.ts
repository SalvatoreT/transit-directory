  ...
  async run(event: Readonly<WorkflowEvent<{}>>, step: WorkflowStep) {
    ...
    while (Date.now() < cutoffTime) {
      ...
      for (let i = 0; i < agencies.length; i++) {
        ...
        console.log(
          `Processing agency: ${agency} (${i + 1}/${agencies.length}) - Iteration ${iteration}`,
        );

        // --- Trip Updates ---
        const tripUpdatesMetadata = await step.do(
          `[${agency}] Sync TripUpdates - Iteration ${iteration}`,
          async () => {
        ...
        if (tripUpdatesMetadata.rateLimitRemaining === 0) {
          await step.sleep(
            `[${agency}] TripUpdates: Rate limit reached, waiting 300s - Iteration ${iteration}`,
            `300 seconds`,
          );
        ...
        await step.sleep(
          `[${agency}] Sleep after TripUpdates - Iteration ${iteration}`,
          `${intervalSeconds} seconds`,
        );

        // --- Service Alerts ---
        const serviceAlertsMetadata = await step.do(
          `[${agency}] Sync ServiceAlerts - Iteration ${iteration}`,
          async () => {
            ...
            for (const entity of message.entity || []) {
              ...
              for (const ie of entities) {
                const routePk = ie.routeId
                  ...
                  : null;
                const stopPk = ie.stopId
                  ? stopMap.get(ie.stopId) || null
                  : null;
        ...
        if (serviceAlertsMetadata.rateLimitRemaining === 0) {
          await step.sleep(
            `[${agency}] ServiceAlerts: Rate limit reached, waiting 300s - Iteration ${iteration}`,
            `300 seconds`,
          );
        ...
        // Sleep after ServiceAlerts to maintain pace
        await step.sleep(
          `[${agency}] Sleep after ServiceAlerts - Iteration ${iteration}`,
          `${intervalSeconds} seconds`,
        );
      }
    }
